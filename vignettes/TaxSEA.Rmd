...
```{r bsdb-available, include = FALSE}
# BugSigDB is downloaded from Zenodo at build time. If that fails, build the
# vignette without the BugSigDB-dependent chunk rather than erroring.
bsdb_available <- requireNamespace("bugsigdbr", quietly = TRUE) &&
  tryCatch({ bugsigdbr::importBugSigDB(); TRUE },
...
#### Output
The output is a list of dataframes: `All_databases` with every set tested, then one
per group of sources -- `Metabolite_producers` (gutMGene, MiMeDB),
`Health_associations` (GMRepoV2, mBodyMap), `BacDive_bacterial_physiology`,
`Gut_Brain_Modules_VallesColomer2019`, and `BugSigDB`, which is empty unless
`bugsigdb = TRUE`.
...
```{r bugsigdb, eval = bsdb_available}
